...
//  COMPLETED (10) Remove the mWeatherDisplay TextView declaration
    ...
    private TextView dateTextview, descTextview, highTextview,
            lowTextview, humidityTextview,
...
//      COMPLETED (13) Find each of the TextViews by ID
        ...
        descTextview = (TextView) findViewById(R.id.weather_description);
        highTextview = (TextView) findViewById(R.id.high_temperature);
        lowTextview = (TextView) findViewById(R.id.low_temperature);
...
//      COMPLETED (17) Throw a NullPointerException if that URI is null
        if (mUri == null) throw new NullPointerException("URI can't be null");
...
//      COMPLETEt (26) Display a readable data string
        ...
        String maxTempAsString = SunshineWeatherUtils.formatTemperature(this,maxTemp);

        highTextview.setText(maxTempAsString);


//      COMPLETED (29) Display the low temperature

        double minTemp = data.getDouble(INDEX_WEATHER_MIN_TEMP);
        String minTempAsString = SunshineWeatherUtils.formatTemperature(this,minTemp);

        lowTextview.setText(minTempAsString);


//      COMPLETED (30) Display the humidity
        float humidity = data.getFloat(INDEX_WEATHER_HUMIDITY);
        String humidityAsString = getString(R.string.format_humidity,humidity);

        humidityTextview.setText(humidityAsString);


//      COMPLETED (31) Display the wind speed and direction
        float windSpeed = data.getFloat(INDEX_WEATHER_WIND_SPEED);
        float windDirection = data.getFloat(INDEX_WEATHER_DEGREES);
        String windAsString = SunshineWeatherUtils.getFormattedWind(this,windSpeed,windDirection);

        windTextview.setText(windAsString);


//      COMPLETED (32) Display the pressure

        float pressure = data.getFloat(INDEX_WEATHER_PRESSURE);
        String pressureAsString = getString(R.string.format_pressure,pressure);
        pressureTextview.setText(pressureAsString);

        //      COMPLETED (33) Store a forecast summary in mForecastSummary
        mForecastSummary = String.format("%s - %s - %s/%s",
                dateText,description,maxTempAsString,minTempAsString);
